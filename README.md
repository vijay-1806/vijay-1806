# Hi there, I'm Vijayaragavan G 👋

<div align="center">

![Cyber Security](https://img.shields.io/badge/Cyber_Security-0d1117?style=for-the-badge&logo=shield&logoColor=blue)
![Python](https://img.shields.io/badge/Python-0d1117?style=for-the-badge&logo=python&logoColor=yellow)
![Cloud](https://img.shields.io/badge/Cloud_Enthusiast-0d1117?style=for-the-badge&logo=icloud&logoColor=white)
![Blockchain](https://img.shields.io/badge/Blockchain-0d1117?style=for-the-badge&logo=ethereum&logoColor=purple)

</div>

---

## 🚀 I Build, Secure, and Scale

I am a **Cybersecurity student & Cloud Enthusiast** with a passion for building real-world systems from scratch. I build systems that solve real problems and secure them against real threats.

- 🎓 **Education:** B.E. Computer Science (Cyber Security) @ Sri Shakthi Institute of Engineering and Technology, Coimbatore (2024–2028)
- 🔭 **Currently working on:** Blockchain Project
- ☁️ **Previously built:** HostVault — Self-hosted Private Cloud Storage Platform
- ⚡ **Fun Fact:** I believe the best way to learn security is to build something and then try to break it.

---

## 🛠️ My Tech Stack

| Category | Skills |
|---|---|
| Languages | Python, C, Java, JavaScript, HTML5/CSS3 |
| Databases | MySQL, MongoDB, PostgreSQL |
| Backend | Flask, Waitress, REST APIs |
| Storage | MinIO (Object Storage) |
| Security | Brute Force Protection, Rate Limiting, Input Validation, Session Hardening |
| DevOps | Git, GitHub, Nginx, Linux |
| Tools | VS Code, pgAdmin, Postman |

---

## 📂 Featured Projects

### ☁️ HostVault — Private Cloud Storage
**Self-hosted Google Drive alternative**
A full-stack private cloud storage platform built with Flask, MinIO, PostgreSQL and Nginx. No third-party cloud — everything runs on your own machine.
- 📤 File upload/download with drag & drop
- 🗑️ Trash system with 30-day recovery
- 🛡️ Admin panel — manage users, files, storage & activity logs
- 🔒 Brute force protection, rate limiting, input validation, session hardening
- 🔐 Google OAuth login support
- 🌙 Dark / Light theme

🔗 [View Repository](https://github.com/vijay-1806/hostvault)

---

### ⛓️ Blockchain Project *(In Progress)*
Currently building a blockchain-based system focused on decentralized data integrity and security.

---

## 📊 GitHub Activity & Stats

<div align="center">

[![GitHub Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=vijay-1806&theme=react-dark&hide_border=true)](https://github.com/vijay-1806)

</div>

<div align="center">

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=vijay-1806&layout=compact&theme=dark&hide_border=true)
![Vijay's GitHub Stats](https://github-readme-stats.vercel.app/api?username=vijay-1806&show_icons=true&theme=dark&hide_border=true)

</div>

<div align="center">

[![GitHub Trophies](https://github-profile-trophy.vercel.app/?username=vijay-1806&theme=darkhub&no-frame=true&row=1)](https://github.com/vijay-1806)

</div>

---

## 📫 Connect with Me

- 📧 **Email:** vijayaragavan183@gmail.com
- 💼 **LinkedIn:** [Vijayaragavan G](https://www.linkedin.com/in/vijayaragavan-g-75591b338)
- 🌐 **Portfolio:** [vijay-1806.github.io](https://vijay-1806.github.io)

---

<div align="center">

*"Build it. Secure it. Scale it."*

</div>
